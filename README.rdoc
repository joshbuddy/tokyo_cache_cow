...
Lets write four keys: _other_key_, _test_key_, _test_key2_ and _test_key3_.
...
Read back _test_key_ and make sure life is still good.

  >> Rails.cache.read('test_key2')
  => "test_value2"

But lets delete _test_key2_ for fun.

  >> Rails.cache.delete('test_key2')
  => true

Confirm that _test_key2_ is really gone.

  >> Rails.cache.read('test_key2')
  => nil

.. but our other keys (namely, _test_key_) are just fine, thank you.

  >> Rails.cache.read('test_key')
  => "test_value"

.. lets nuke EVERYTHING with _test_key_ in it though.

  >> Rails.cache.delete_matched('test_key')
  => true

Now _test_key_ and _test_key3_ are both nuked.
...
But _other_key_ is still peachy.
